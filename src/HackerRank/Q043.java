...
which is like a regular queue, but where additionally each element
...
1.The student having the highest
   Cumulative Grade Point Average (CGPA) is served first.
2.Any students having the same CGPA will be served by name
  in ascending case-sensitive alphabetical order.
3.Any students having the same CGPA and name will be served in
...
to process all the given events and return all the students
...
The locked stub code prints the names of the students
yet to be served in the priority order.
...
class Priorities{
    ...
    public static List<Student2> getStudents(List<String> events){
        PriorityQueue<Student2> stdList = new PriorityQueue<Student2>();
        List<Student2> rtr = new ArrayList<>();
        String parsedStr[] = null;
        for(String w:events){
            if (w.equals("SERVED")){
                if (stdList.peek()!=null){
                    stdList.poll();
                }
            }else{
                parsedStr = w.trim().split(" ");
                if (parsedStr[0].equals("ENTER")){
                    stdList.add(new Student2(parsedStr[1],Double.parseDouble(parsedStr[2]),
                                              Integer.parseInt(parsedStr[3])
                                            ));
                }

            }
        }
        while(stdList.peek()!=null){
            rtr.add(stdList.poll());
        }

        return rtr;
    }
    public static List<Student2> getStudents2(List<String> events){
        //If you dont want to implements Comparable interface in Student then
        ...
        for(String w:events){
            if (w.equals("SERVED")){
                ...
                /*
                Scanner line = new Scanner(w);
                String op = line.next();
                String name = line.next();
                double cgpa = line.nextDouble();
                int pri = line.nextInt();
